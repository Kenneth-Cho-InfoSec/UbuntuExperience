...
const defaultData = {
  ...
  gallery: [
    { id: 'ref-apps', src: '/references/apps-list.png', label: 'App drawer reference', album: 'Screenshots', favorite: true, deleted: false, date: '2026-05-01' },
    { id: 'ref-device', src: '/references/pinephone-pro.jpg', label: 'PinePhone Pro', album: 'Devices', favorite: false, deleted: false, date: '2026-04-28' },
    { id: 'ref-lock', src: '/references/lockscreen.jpg', label: 'Lock screen reference', album: 'Screenshots', favorite: false, deleted: false, date: '2026-05-01' },
  ],
  contacts: [
  ...
  return (
    <nav className={`launcher ${visible ? 'visible' : 'hidden'}`} aria-label="Ubuntu launcher" aria-hidden={!visible}>
      ...
      <button className={`ubuntu-home ${showDrawer ? 'drawer-open' : ''}`} onClick={goHome} aria-label={showDrawer ? 'Close applications' : 'Open applications'} title={showDrawer ? 'Close applications' : 'Open applications'}>
        <img src="/ubuntu-cof.svg" alt="Ubuntu home" />
      </button>
    </nav>
  )
}
...
function TaskPreview({ app }) {
  if (app.id === 'gallery') {
    return <div className="preview-gallery"><div className="calendar-chip">Photos<br /><strong>3</strong></div><img src="/references/lockscreen.jpg" alt="" /><div className="calendar-chip muted">Albums</div></div>
  }
...
function CameraApp({ data, updateData, notify }) {
  ...
  const capture = () => {
    let src = '/references/lockscreen.jpg'
